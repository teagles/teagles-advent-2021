...
from teagles_advent_2021.day_9.lib import parse_input, valid, Point, neighbors, TargetAndNeighbors, low_point, risk,\
    points_with_neighbors, get_basin
...
"""))
    ...
    def test_neighbors(self):
        test_matrix = [[2, 1, 9, 9, 9, 4, 3, 2, 1, 0],
                       ...
                       [8, 7, 6, 7, 8, 9, 6, 7, 8, 9],
                       [9, 8, 9, 9, 9, 6, 5, 6, 7, 8]]
        expectation = TargetAndNeighbors(2, frozenset([1, 3]), Point(0, 0))
        reality = neighbors(test_matrix, Point(0, 0))
        self.assertEqual(expectation, reality)
        expectation = TargetAndNeighbors(8, frozenset([7, 9]), Point(4, 9))
        reality = neighbors(test_matrix, Point(4, 9))
        self.assertEqual(expectation, reality)
        expectation = TargetAndNeighbors(3, frozenset([2, 9]), Point(1, 0))
        reality = neighbors(test_matrix, Point(1, 0))
        self.assertEqual(expectation, reality)
        expectation = TargetAndNeighbors(9, frozenset([6, 8, 9]), Point(4, 2))
        reality = neighbors(test_matrix, Point(4, 2))
        self.assertEqual(expectation, reality)

    def test_low_point(self):
        self.assertEqual(True, low_point(TargetAndNeighbors(0, frozenset([1, 9]), None)))
        self.assertEqual(False, low_point(TargetAndNeighbors(9, frozenset([1, 8, 9]), None)))

    def test_risk(self):
        self.assertEqual(1, risk(TargetAndNeighbors(0, frozenset([1, 9]), None)))
        self.assertEqual(0, risk(TargetAndNeighbors(9, frozenset([1, 8, 9]), None)))

    def test_points_with_neighbors(self):
        test_matrix = [[0, 1],
                       [1, 0]]
        expectation = [TargetAndNeighbors(target=0, neighbors=frozenset({1}), co_ords=Point(0, 0)),
                       TargetAndNeighbors(target=1, neighbors=frozenset({0}), co_ords=Point(0, 1)),
                       TargetAndNeighbors(target=1, neighbors=frozenset({0}), co_ords=Point(1, 0)),
                       TargetAndNeighbors(target=0, neighbors=frozenset({1}), co_ords=Point(1, 1))]
    ...
    def test_get_basin(self):
        test_matrix = [[2, 1, 9, 9, 9, 4, 3, 2, 1, 0],
                       ...
                       [8, 7, 6, 7, 8, 9, 6, 7, 8, 9],
                       [9, 8, 9, 9, 9, 6, 5, 6, 7, 8]]
        expectation = {Point(0, 1), Point(1, 0), Point(0, 0)}
        reality = get_basin(test_matrix, Point(0, 1))
        self.assertEqual(expectation, reality)
